...
public class BannerInheritanceProvider implements InheritanceProvider {
    ...
    protected Set<String> fineNMSParents(String className, boolean remap) {
        if (remap) {
            className = RemapUtilProvider.get().map(className);
        }
        ...
        try {
            ...
            if (extend != null) {
                parents.add(RemapUtilProvider.get().reverseMap(extend));
            }
            for (Class<?> inter : reference.getInterfaces()) {
                if (inter != null) {
                    parents.add(RemapUtilProvider.get().reverseMap(inter));
                }
            }
    ...
    protected Set<String> findNormalParents(String className, boolean remap) {
        ...
        if (cn == null) {
            ...
            String remapClassName = RemapUtilProvider.get().map(className);
        ...
        if (cn.superName != null) {
            parents.add(RemapUtilProvider.get().reverseMap(cn.superName));
        }
        if (cn.interfaces != null) {
            for (String anInterface : cn.interfaces) {
                parents.add(RemapUtilProvider.get().reverseMap(anInterface));
            }
        }
