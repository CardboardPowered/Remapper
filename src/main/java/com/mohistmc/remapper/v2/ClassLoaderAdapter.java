...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper) {
            ...
            for (AbstractInsnNode insnNode : methodNode.instructions) {
                if (insnNode.getOpcode() == Opcodes.NEW) {
                    ...
                    if (replace != null) {
                        ...
                        MohistRemapper.LOGGER.debug(MARKER, "Found new {}/{} call in {} {}", typeInsnNode.desc, ((MethodInsnNode) next).name + ((MethodInsnNode) next).desc, node.name, methodNode.name + methodNode.desc);
        ...
        MohistRemapper.LOGGER.debug(MARKER, "Transforming classloader class {}", node.name);
        if (!info.remapping) {
            implementIntf(node);
        }
        for (MethodNode methodNode : node.methods) {
    ...
    private void implementIntf(ClassNode node) {
        MohistRemapper.LOGGER.debug(MARKER, "Implementing RemappingClassLoader for class {}", node.name);
        ...
        list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(MohistRemapper.class), "createClassLoaderRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class), Type.getType(ClassLoader.class)), false));
