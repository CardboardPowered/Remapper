...
import com.mohistmc.remapper.utils.ArrayUtil;
...
public class MohistRedirectAdapter implements PluginTransformer {
    ...
    public static final MohistRedirectAdapter INSTANCE = new MohistRedirectAdapter();
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        MohistRemapper.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    MohistRemapper.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));
    ...
    private static void addRule(boolean modifyArgs, Class<?> owner, String name, String handlerName, Class<?>... args) {
        ...
        Method handler = methodOf(MohistReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        while (handler == null) {
            handlerArgs[0] = handlerArgs[0].getSuperclass();
            handler = methodOf(MohistReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        }
        ...
        if (modifyArgs) {
            Method modifyHandler = methodOf(MohistReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            if (modifyHandler == null) {
                handlerArgs[0] = original.getReturnType();
                modifyHandler = methodOf(MohistReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            }
            if (modifyHandler == null) {
